...
export default function JobCard({ job }: JobCardProps) {
    ...
    async function analyzeJobPosting() {
       const analysis = await analyzeJob(job.title, job.description);
    ...
    return  (
        <div className="flex flex-col gap-2 h-full rounded-lg bg-emerald-100 p-6 shadow-md">
            ...
            <a href={job.applyUrl} target="_blank" rel="noopener noreferrer" className="rounded-md bg-white mt-auto shrink-0 px-2 py-2 gap-2 hover:bg-emerald-300 text-center" onClick={analyzeJobPosting}>Apply</a>
        </div>
    );
}
